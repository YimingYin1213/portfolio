...
class GameLevelAquaticGameLevel {
    constructor(gameEnv) {
        ...
        const audioAssetPath = path + '/assets/audio';
        const assetPath = spriteAssetPath;
        const kirbyMinigamesAssetPath = spriteAssetPath;
        const bubbleSpriteSrc = `data:image/svg+xml;utf8,${encodeURIComponent(`
            <svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 96 96">
                <circle cx="24" cy="26" r="16" fill="rgba(133,238,255,0.18)" stroke="rgba(210,250,255,0.9)" stroke-width="2" />
                <circle cx="24" cy="20" r="5" fill="rgba(255,255,255,0.45)" />
                <circle cx="68" cy="56" r="19" fill="rgba(133,238,255,0.14)" stroke="rgba(210,250,255,0.82)" stroke-width="2" />
                <circle cx="62" cy="49" r="6" fill="rgba(255,255,255,0.4)" />
                <circle cx="52" cy="18" r="9" fill="rgba(133,238,255,0.12)" stroke="rgba(210,250,255,0.74)" stroke-width="2" />
            </svg>
        `)}`;
        const megalodonDeathSheetSrc = null;
        const width = gameEnv.innerWidth;
        const height = gameEnv.innerHeight;
        const aquaticSpriteStorageKey = 'aquatic_selected_sprite_v1';
        const scubaBaseSheet = {
            src: kirbyMinigamesAssetPath + '/Scuba Diver V2.png',
            pixels: { height: 145, width: 1158 },
            orientation: { rows: 1, columns: 6 }
        };
        const scubaThrowSheet = {
            src: kirbyMinigamesAssetPath + '/Scuba Diver Row 2.png',
            pixels: { height: 151, width: 712 },
            orientation: { rows: 1, columns: 1 }
        };
        const scubaMovementSheet = {
            src: kirbyMinigamesAssetPath + '/Scuba Diver Row 3.png',
            pixels: { height: 127, width: 1105 },
            orientation: { rows: 1, columns: 1 }
        };
        const scubaIdleSheet = {
        ...
        const aquaticSpriteOptions = [
            {
                key: 'scuba-diver',
                label: 'Scuba Diver',
                src: scubaBaseSheet.src,
                pixels: { ...scubaBaseSheet.pixels },
                SCALE_FACTOR: 5,
                ANIMATION_RATE: 8,
                orientation: { ...scubaBaseSheet.orientation },
                down: { row: 0, start: 0, columns: 6 },
                downRight: { row: 0, start: 0, columns: 6 },
                downLeft: { row: 0, start: 0, columns: 6, mirror: true },
                left: { row: 0, start: 0, columns: 6, mirror: true },
                right: { row: 0, start: 0, columns: 6 },
                up: { row: 0, start: 0, columns: 6 },
                upLeft: { row: 0, start: 0, columns: 6, mirror: true },
                upRight: { row: 0, start: 0, columns: 6 },
                customAnimator: {
                    movementRight: {
                        src: scubaMovementSheet.src,
                        pixels: { ...scubaMovementSheet.pixels },
                        orientation: { ...scubaMovementSheet.orientation },
                    ...
                    throw: {
                        src: scubaThrowSheet.src,
                        pixels: { ...scubaThrowSheet.pixels },
                        orientation: { ...scubaThrowSheet.orientation },
                        frames: scubaThrowFrames
                    },
                    idle: {
                        src: scubaIdleSheet.src,
                        pixels: { ...scubaIdleSheet.pixels },
                        orientation: { ...scubaIdleSheet.orientation },
        ...
        this.underwaterMusicSrc = audioAssetPath + '/Megalodon Boss Fight.mp3';
        ...
        const applyAquaticPlayerSprite = (spriteOption) => {
            ...
            ensureAquaticCustomFrameSupport(player);
        ...
        const ensureAquaticCustomFrameSupport = (player) => {
            if (!player || player._aquaticCustomFramesWrapped) return;

            const getDirectionFrames = () => {
                const directionData = player.spriteData?.[player.direction] || {};
                return Array.isArray(directionData.frames) && directionData.frames.length
                    ? directionData.frames
                    : null;
            };

            const originalGetFrameDimensions = player.getFrameDimensions?.bind(player);
            const originalGetFrameAspectRatio = player.getFrameAspectRatio?.bind(player);
            const originalDrawSprite = player.drawSprite?.bind(player);
            const originalUpdateAnimationFrame = player.updateAnimationFrame?.bind(player);

            player.getFrameDimensions = () => {
                const frames = getDirectionFrames();
                if (!frames || !originalGetFrameDimensions) return originalGetFrameDimensions?.() || { frameWidth: 1, frameHeight: 1 };

                const pixels = player.spriteData?.pixels || { width: player.spriteSheet?.naturalWidth || 1, height: player.spriteSheet?.naturalHeight || 1 };
                const orientation = player.spriteData?.orientation || { rows: 1, columns: 1 };
                const frameHeight = Math.max(1, Math.round(pixels.height / Math.max(1, orientation.rows || 1)));
                const safeIndex = ((player.frameIndex || 0) % frames.length + frames.length) % frames.length;
                const currentFrame = frames[safeIndex] || frames[0];

                return {
                    frameWidth: Math.max(1, Math.round(currentFrame.width || 1)),
                    frameHeight
                };
            };

            player.getFrameAspectRatio = () => {
                const frames = getDirectionFrames();
                if (!frames || !originalGetFrameAspectRatio) return originalGetFrameAspectRatio?.() || 1;
                const { frameWidth, frameHeight } = player.getFrameDimensions();
                return frameHeight > 0 ? frameWidth / frameHeight : 1;
            };

            player.drawSprite = () => {
                const frames = getDirectionFrames();
                if (!frames || !originalDrawSprite) {
                    originalDrawSprite?.();
                    return;
                }

                if (!player.spriteReady) {
                    player.drawDefaultSquare?.();
                    return;
                }
                ...
                player.canvas.width = frameWidth;
                player.canvas.height = frameHeight;

                player.applyTransformations?.(directionData);
                player.applyFilters?.(directionData);
                player.ctx.drawImage(
                    player.spriteSheet,
                    Math.max(0, Math.round(currentFrame.x || 0)),
                    0,
                    frameWidth,
                    frameHeight,
                    0,
            ...
            player.updateAnimationFrame = () => {
                const frames = getDirectionFrames();
                if (!frames || !originalUpdateAnimationFrame) {
                    originalUpdateAnimationFrame?.();
                    return;
                }

                player.frameCounter += 1;
                if (player.frameCounter % Math.max(1, player.animationRate || 1) === 0) {
                    player.frameIndex = (player.frameIndex + 1) % frames.length;
                }
            };

            player._aquaticCustomFramesWrapped = true;
        };

        this.ensureAquaticCustomFrameSupport = ensureAquaticCustomFrameSupport;
        ...
        const ensureFrontMenuKeyframes = () => {
            ...
            style.textContent = `
                ...
                @keyframes aquaticBubbleDriftFast {
                    from { transform: translate3d(0, 0, 0); }
                    to { transform: translate3d(-110px, -68px, 0); }
                }
        ...
        const showFrontMenu = () => {
            ...
            const makeBubbleLayer = (opacity, duration, size, animationName) => {
                ...
                Object.assign(layer.style, {
                    ...
                    mixBlendMode: 'screen',
                    filter: 'drop-shadow(0 0 5px rgba(124, 245, 255, 0.18))',
            ...
            overlay.appendChild(makeBubbleLayer(0.1, 36, 220, 'aquaticBubbleDriftSlow'));
            overlay.appendChild(makeBubbleLayer(0.14, 24, 320, 'aquaticBubbleDriftFast'));
        ...
        const transitionBackUnderwater = async () => {
            ...
            try {
                ...
                setBackground(backgroundAssetPath + '/Aquatic.png');
    ...
    initialize() {
        ...
        if (player && !player._aquaticLockWrapped) {
            this.ensureAquaticCustomFrameSupport?.(player);
