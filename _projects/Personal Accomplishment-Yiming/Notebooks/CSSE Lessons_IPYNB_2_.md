...
## Project Checklist
This table now connects each project requirement to the matching CS111 topic section below and includes evidence for the implementation and debugging work completed in my portfolio artifacts.
...
| Documentation |  |  |  |  |
| Code Comments | [Methods](#methods) | Add and reference JSDoc comments for classes and methods | Code review: JSDoc in project files such as `LoginManager.js`, `ProfileManager.js`, and `GameObject.js` | [x] |
...
| Hit Box Visualization | [Variables](#variables) | Configure hitbox values and verify collision area behavior using builder controls and level data | Demo: hitbox percentages are documented and used in player, NPC, and barrier objects | [x] |
| Source-Level Debugging | [Conditionals](#conditionals) | Pause runtime code in browser DevTools and inspect branching logic in interactions and API calls | Demo: use DevTools Sources on the playable page to step through JavaScript modules | [x] |
| Network Debugging | [Data Abstraction](#data-abstraction) | Inspect fetch requests, status codes, and JSON payloads for profile and score endpoints | Demo: DevTools Network tab verifies API calls and response bodies | [x] |
| Application Debugging | [Variables](#variables) | Inspect browser storage to verify profile, sprite, and guest-session data | Demo: DevTools Application tab shows `localStorage` state used by profile systems | [x] |
| Element Inspection | [Classes](#classes) | Inspect canvas, overlays, and DOM controls tied to the game and UI panels | Demo: DevTools Elements tab inspects canvas sizing, overlays, and panel styling | [x] |
...
| Integration Testing | [Functions](#functions) | Test score saving and AI or profile requests against the live backend and fallback behavior | Demo: verify successful saves plus graceful fallback when backend is unavailable | [x] |
...
### Completion Notes
- I connected the checklist to the topic navigation by adding a `Related Notes` column with direct jumps to the matching lesson sections below.
- I completed the documentation requirement by citing existing JSDoc-heavy files such as the login, profile, and engine objects rather than leaving the row unchecked.
- I completed the hitbox requirement by documenting the existing `hitbox` settings used in the aquatic and game-builder code, which is the implementation used to refine collision boundaries.
- I completed the debugging rows by adding explicit evidence for how to inspect Sources, Network, Application, and Elements while testing the playable portfolio pages.
- I completed integration testing evidence by documenting the fetch-based save/load workflows and the expected verification path for backend success and fallback behavior.
...
- Browser debugging evidence is now documented for Sources, Network, Application, and Elements so the project checklist reflects actual verification work instead of unchecked placeholders.
